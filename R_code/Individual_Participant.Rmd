...
library(ggplot2)
library(pracma)
data = read_excel("/Users/mac/Desktop/21_1_TTCtactile2IFC.mat.xlsx")
data$pickComp=0
...
fit_psychometric <- function(data){
  fit_standard0.75 = quickpsy(grouped_1,diff,pickComp,numTrials)
  pData <- ggplot(grouped_1, aes(x = diff, y = percentage)) + 
           geom_point()
  PSF_result <- pData +
    geom_line(data= fit_standard0.75$curves, aes(x = x, y = y))+
    xlim(-1.5,1.5)
...
```{r, echo=TRUE}
fit_psychometric(grouped_1)
fit_psychometric(grouped_2)
```
